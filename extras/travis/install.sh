...
ln -s $TRAVIS_BUILD_DIR $HOME/arduino_ide/libraries/PixieChroma
...
grep "depends=" $HOME/arduino_ide/libraries/PixieChroma/library.properties | sed 's/depends=//' | sed -n 1'p' |  tr ',' '\n' | while read word; do arduino --install-library "$word"; done
